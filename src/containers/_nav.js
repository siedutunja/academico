...
jwt.verify(token, CONFIG.SECRET_KEY, (err, data) => {
  if (err) {
    ...
    if (tokenDecodificado.id_rol == 1 || tokenDecodificado.id_rol == 12 || tokenPermisos.perMatricular == 1) {
      ...
      menu[0]._children[indice].items.push({ name: 'Renovación Matricula Masiva', to: '/matriculas/renovacionmasiva'})
    ...
    menu[0]._children[indice].items.push({ name: 'Planilla Manual de Notas', to: '/listados/planillamanualparcial'})
    ...
    if (tokenDecodificado.id_rol == 1 || tokenDecodificado.id_rol == 12 || tokenPermisos.perActNotas == 1) {
      ...
      if (tokenDecodificado.id_rol == 1 || tokenDecodificado.id_rol == 12 || tokenPermisos.perActNotas == 1) {
        ...
        //menu[0]._children[indice].items.push({ name: '* Notas Asignatura', to: '/calificaciones/planillanotasasignatura'})
      }
    }
    if (tokenDecodificado.id_rol == 1 || tokenDecodificado.id_rol == 12) {
      menu[0]._children.push(
