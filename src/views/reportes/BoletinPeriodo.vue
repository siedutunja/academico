<template>
  <div>
    <button class="btn btn-success float-right btn-block" @click="generarPDF">📄 Imprimir Boletines o Generar Boletines en PDF</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    notaFinalArea(est, area) {
      ...
      if (orden == 99 && this.tipoValComp == 0) return this.promedioAreaPorPeriodo(est, area, this.periodoActual) //this.promedioAsignatura(est, area, asignatura)
    ...
    calcularPromedioGeneralPorAreasFinales(est) {
      const areasEvaluativas = [...new Set(
