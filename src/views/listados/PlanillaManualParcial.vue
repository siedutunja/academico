  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> PLANILLA MANUAL DE NOTAS PARCIALES</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="2">
                ...
                <div v-else>
                  ...
                  :select-options="{enabled: true,selectionText: 'estudiantes seleccionados',clearSelectionText: 'Limpiar',}">
                    <template #selected-row-actions>
                      <button class="small btn btn-primary" @click="imprimirReportes()">Imprimir Planillas</button>
                    </template>
                    <div slot="emptystate">
          ...
          <template #footer>
            <em>Planillas manual de evaluación por periodo.</em>
          </template>
        </b-card>
      </b-col>
    </b-row>
  ...
  export default {
    ...
    methods: {
      async imprimirReportes() {
        ...
        window.open("https://siedutunja.gov.co/php/listas/planillamanual.php" + encoded,"_blank")
        return true
      },
