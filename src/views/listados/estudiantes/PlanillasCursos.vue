  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> PLANILLAS DE ESTUDIANTES POR CURSO</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="12">
                          ...
                          <div ref="contenido">
                            <div v-for="(curso, index) in cursosSeleccionados" :key="curso.idCurso" class="bloque-curso">
                              <div class="encabezado">
                                <p>SECRETARÍA DE EDUCACIÓN TERRITORIAL DE TUNJA<br><b>{{$store.state.nombreInstitucion}}</b><br>TUNJA - BOYACÁ<br>PLANILLAS DE ESTUDIANTES POR CURSO<br></p>
                              </div>
  ...
  export default {
    ...
    methods: {
      ...
      async cargarDataEstudiantes() {
        ...
        .catch(err => {
          this.mensajeEmergente('danger',CONFIG.TITULO_MSG,'Algo salio mal y no se pudo realizar: Consulta Data Estudiantes. Intente más tarde.' + err)
          this.btnCargando = false
        })
        this.btnCargando = false
      },
