  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> LISTA DE ESTUDIANTES NUEVOS EN LA I.E.</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="12">
                        ...
                        <div class="informe-estudiantes">
                          <div ref="contenido">
                            <div class="encabezado">
                              <p>SECRETARÍA DE EDUCACIÓN TERRITORIAL DE TUNJA<br><b>{{$store.state.nombreInstitucion}}</b><br>TUNJA - BOYACÁ<br>ESTUDIANTES NUEVOS EN LA I.E. - AÑO LECTIVO {{$store.state.aLectivo}}<br></p>
                            </div>
  ...
  export default {
    ...
    methods: {
      ...
      imprimir() {
        ...
        `
        const ventana = window.open('Lista Nuevos', '_blank')
        ventana.document.write(`
          <html>
            <head><title>Lista Nuevos</title>${estilos}</head>
            <body">${contenido}</body>
          </html>
        `)
      ...
      exportarAExcel() {
        ...
        XLSX.writeFile(wb, 'Nuevos.xlsx')
      },
      async cargarDataEstudiantes() {
        this.btnCargando = true
        this.dataConsultada = []
        await axios
        .get(CONFIG.ROOT_PATH + 'academico/data/estudiantes/nuevos', { params: { idInstitucion: this.$store.state.idInstitucion, vigencia: this.$store.state.aLectivo }})
        .then(response => {
