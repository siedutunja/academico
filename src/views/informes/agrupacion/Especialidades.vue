  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> LISTA DE ESTUDIANTES POR ESPECIALIDAD</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="12">
            ...
            <b-row>
              <b-col lg="6">
                <b-form-group label="Seleccione la Especialidad:" label-for="especialidad" class="etiqueta">
                  <b-form-select id="especialidad" ref="especialidad" v-model="idEspecialidad" :options="comboEspecialidades"></b-form-select>
                </b-form-group>
              </b-col>
            </b-row>
            <b-row v-if="idEspecialidad">
                        ...
                        <div class="informe-estudiantes">
                          <div ref="contenido">
                            <div class="encabezado">
                              <p>SECRETARÍA DE EDUCACIÓN TERRITORIAL DE TUNJA<br><b>{{$store.state.nombreInstitucion}}</b><br>TUNJA - BOYACÁ<br>ESTUDIANTES POR ESPECIALIDAD - AÑO LECTIVO {{$store.state.aLectivo}}<br></p>
                            </div>
    ...
    methods: {
      estudiantesAgrupados() {
        ...
        return this.dataConsultada.filter(e =>
          e.idEspecialidad === this.idEspecialidad
        )
      },
      imprimir() {
        ...
        `
        const ventana = window.open('Lista por Especialidad', '_blank')
        ventana.document.write(`
          <html>
            <head><title>Lista por Especialidad</title>${estilos}</head>
            <body">${contenido}</body>
          </html>
        `)
      ...
      exportarAExcel() {
        ...
        XLSX.writeFile(wb, 'EstudiantesPorEspecialidades.xlsx')
      },
      ...
      async cargarDataEstudiantes() {
        ...
        this.dataConsultada = []
        await axios
        .get(CONFIG.ROOT_PATH + 'academico/data/estudiantes/especialidades', { params: { idInstitucion: this.$store.state.idInstitucion, vigencia: this.$store.state.aLectivo }})
        .then(response => {
          if (response.data.error){
