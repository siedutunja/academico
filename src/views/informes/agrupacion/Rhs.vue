  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> LISTA DE ESTUDIANTES POR GRUPO SANGUINEO - RH</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="12">
            ...
            <b-row>
              <b-col lg="6">
                <b-form-group label="Seleccione el Grupo:" label-for="rh" class="etiqueta">
                  <b-form-select id="rh" ref="rh" v-model="idRh" :options="comboRhs"></b-form-select>
                </b-form-group>
              </b-col>
            </b-row>
            <b-row v-if="idRh">
                        ...
                        <div class="informe-estudiantes">
                          <div ref="contenido">
                            <div class="encabezado">
                              <p>SECRETARÍA DE EDUCACIÓN TERRITORIAL DE TUNJA<br><b>{{$store.state.nombreInstitucion}}</b><br>TUNJA - BOYACÁ<br>ESTUDIANTES POR GRUPO SANGUINEO - RH - AÑO LECTIVO {{$store.state.aLectivo}}<br></p>
                            </div>
  ...
  export default {
    ...
    methods: {
      ...
      imprimir() {
        ...
        `
        const ventana = window.open('Lista por Rh', '_blank')
        ventana.document.write(`
          <html>
            <head><title>Lista por Rh</title>${estilos}</head>
            <body">${contenido}</body>
          </html>
        `)
      ...
      exportarAExcel() {
        ...
        XLSX.writeFile(wb, 'EstudiantesPorRh.xlsx')
      },
      ...
      async cargarDataEstudiantes() {
        ...
        this.dataConsultada = []
        await axios
        .get(CONFIG.ROOT_PATH + 'academico/data/estudiantes/rhs', { params: { idInstitucion: this.$store.state.idInstitucion, vigencia: this.$store.state.aLectivo }})
        .then(response => {
          if (response.data.error){
            ...
            if(response.data.datos != 0) {
              ...
              //console.log(this.dataConsultada)
            }
            this.btnCargando = false
          }
        })
