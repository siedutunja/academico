  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> LISTA DE ESTUDIANTES POR ESTRATO</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="12">
            ...
            <b-row>
              <b-col lg="6">
                <b-form-group label="Seleccione el Estrato:" label-for="estrato" class="etiqueta">
                  <b-form-select id="estrato" ref="estrato" v-model="idEstrato" :options="comboEstratos"></b-form-select>
                </b-form-group>
              </b-col>
            </b-row>
            <b-row v-if="idEstrato != null">
                        ...
                        <div class="informe-estudiantes">
                          <div ref="contenido">
                            <div class="encabezado">
                              <p>SECRETARÍA DE EDUCACIÓN TERRITORIAL DE TUNJA<br><b>{{$store.state.nombreInstitucion}}</b><br>TUNJA - BOYACÁ<br>ESTUDIANTES POR ESTRATO - AÑO LECTIVO {{$store.state.aLectivo}}<br></p>
                            </div>
  ...
  export default {
    ...
    methods: {
      ...
      imprimir() {
        ...
        `
        const ventana = window.open('Lista por Estrato', '_blank')
        ventana.document.write(`
          <html>
            <head><title>Lista por Estrato</title>${estilos}</head>
            <body">${contenido}</body>
          </html>
        `)
