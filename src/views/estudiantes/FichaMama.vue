  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> DATOS DE LA MADRE</h5>
          </template>
          <b-card-text>
            ...
            <b-row v-else>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Número de Documento*" label-for="doc" class="etiqueta">
                  <b-form-input id="doc" ref="doc" v-model="infoMama.documento" aria-describedby="feedDoc" autocomplete="off" maxlength="20" @keydown="soloNumeros" :disabled="mamaExiste ? false : true"></b-form-input>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Género*" label-for="genero" class="etiqueta">
                  <b-form-select  id="genero" ref="genero" v-model="$v.infoMama.id_genero.$model" :options="comboGeneros" :state="validateStateD('id_genero')" aria-describedby="feedGenero"></b-form-select>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Grupo y Factor RH*" label-for="rh" class="etiqueta">
                  <b-form-select  id="rh" ref="rh" v-model="$v.infoMama.id_rh.$model" :options="comboRhs" :state="validateStateD('id_rh')" aria-describedby="feedRh"></b-form-select>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Cabeza Familia*" label-for="cabeza" class="etiqueta">
                  <b-form-select  id="cabeza" ref="cabeza" v-model="$v.infoMama.id_cabeza.$model" :options="comboCabeza" :state="validateStateD('id_cabeza')" aria-describedby="feedCabeza"></b-form-select>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Familias Acción*" label-for="familias" class="etiqueta">
                  <b-form-select  id="familias" ref="familias" v-model="$v.infoMama.id_familias.$model" :options="comboFamilias" :state="validateStateD('id_familias')" aria-describedby="feedFamilias"></b-form-select>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Escolaridad*" label-for="escolar" class="etiqueta">
                  <b-form-select  id="escolar" ref="escolar" v-model="$v.infoMama.id_escolaridad.$model" :options="comboEscolaridades" :state="validateStateD('id_escolaridad')" aria-describedby="feedEscolar"></b-form-select>
              ...
              <b-col lg="3" md="6">
                <b-form-group label="Actividad*" label-for="actividad" class="etiqueta">
                  <b-form-select  id="actividad" ref="actividad" v-model="$v.infoMama.id_actividad.$model" :options="comboLabores" :state="validateStateD('id_actividad')" aria-describedby="feedActiv"></b-form-select>
          ...
          <template #footer>
            ...
            <div v-else>
              <b-button class="small" variant="primary" @click="validarDatosFormulario">
                <div v-if="infoMama.editarMama">Actualizar Datos de la Madre</div>
                <div v-else>Guardar y Asociar la Madre a la Matricula</div>
              </b-button>
              <b-button class="small ml-3" variant="secondary" @click="cancelarFormulario">Cancelar</b-button>
            </div>
          </template>
  ...
  export default {
    ...
    methods: {
      async validarDocumento() {
        ...
        if (cadena.length > 6) {
          await axios
          .get(CONFIG.ROOT_PATH + 'academico/mamas/documento', { params: { documento: this.nuevoDocumento }})
          .then(response => {
            if (response.data.error){
              this.mensajeEmergente('danger',CONFIG.TITULO_MSG,response.data.mensaje + ' - Validar documento madre')
            } else{
              if (response.data.datos != 0) {
                ...
                this.infoMama.documento = this.nuevoDocumento
                this.infoMama.mamaExiste = this.mamaExiste
                this.infoMama.idEstudiante = this.datosMama.idEstudiante
                this.infoMama.id_tipo_documento = 1
                this.infoMama.id_municipio_documento = '15001'
                this.infoMama.id_nacionalidad = '170'
                this.infoMama.id_rh = 0
                this.infoMama.id_cabeza = 0
                this.infoMama.id_familias = 0
                this.infoMama.id_municipio_direccion = '15001'
                this.infoMama.id_escolaridad = 1
                this.infoMama.id_actividad = 2
              }
              this.documentoValido = true
            }
          })
          .catch(err => {
              this.mensajeEmergente('danger',CONFIG.TITULO_MSG,'Algo salio mal y no se pudo realizar: Validar documento madre. Intente más tarde. ' + err)
          })
        } else {
          this.nuevoDocumento = ''
      ...
      validarDatosFormulario() {
        ...
        if (this.$v.infoMama.$anyError) {
          ...
          let titulo = this.infoMama.editarMama ? 'Actualizar Datos de la Madre' : 'Guardar y Asociar Madre'
          let pregunta = this.infoMama.editarMama ? '¿Esta seguro de Actualizar los Datos de la Madre?' : '¿Esta seguro de Guardar y Asociar la Madre a la Matricula?'
          ...
          .then(value => {
            if (value) {
              this.guardarDatosMadre()
            }
          })
        }
        return true
      },
      async guardarDatosMadre() {
        ...
        await axios
        .post(CONFIG.ROOT_PATH + 'academico/mamas', JSON.stringify(this.infoMama), { headers: {"Content-Type": "application/json; charset=utf-8" }})
        .then(response => {
          if (response.data.error){
            this.mensajeEmergente('danger',CONFIG.TITULO_MSG,response.data.mensaje + ' - Crear Madre')
          } else{
            this.$emit("retorno", 1)
          }
        })
        .catch(err => {
          this.mensajeEmergente('danger',CONFIG.TITULO_MSG,'Algo salio mal y no se pudo realizar: Crear Madre. Intente más tarde. ' + err)
        })
      },
      consultaDatosMama() {
        if (this.datosMama.editarMama) {
          this.infoMama = this.datosMama
          this.documentoValido = true
          this.mamaExiste = true
        }
      },
