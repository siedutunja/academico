  ...
  <div>
    <b-row class="mt-2">
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="card-checklist" aria-hidden="true"></b-icon> ACTIVAR/DESACTIVAR PERIODOS PARA RECUPERACIONES</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col lg="6">
    ...
    <b-modal ref="modalSeleccionarFechas" size="" scrollable hide-footer title="Seleccionar Fechas" ok-only>
      <div class="mx-3">
        <div>
          <b-row>
            <b-col lg="12">
              <p>Esta opción permite seleccionar y asignar a los docentes las fechas de Inicio y Cierre del periodo para que carguen las recuperaciones.</p>
            </b-col>
          </b-row>
          <b-row>
            <b-col lg="12">
    ...
    methods: {
      ...
      async cargueListaDocentes() {
        ...
        this.$store.state.datosDocentes.forEach(element => {
          ...
          let fechitaF = null
          if (this.idPeriodo == 1) {
            if (element.recupP1_Ini != null) fechitaI = element.recupP1_Ini.substr(0,10)
            if (element.recupP1_Fin != null) fechitaF = element.recupP1_Fin.substr(0,10)
          } else if (this.idPeriodo == 2) {
            if (element.recupP2_Ini != null) fechitaI = element.recupP2_Ini.substr(0,10)
            if (element.recupP2_Fin != null) fechitaF = element.recupP2_Fin.substr(0,10)
          } else if (this.idPeriodo == 3) {
            if (element.recupP3_Ini != null) fechitaI = element.recupP3_Ini.substr(0,10)
            if (element.recupP3_Fin != null) fechitaF = element.recupP3_Fin.substr(0,10)
          } else if (this.idPeriodo == 4) {
            if (element.recupP4_Ini != null) fechitaI = element.recupP4_Ini.substr(0,10)
            if (element.recupP4_Fin != null) fechitaF = element.recupP4_Fin.substr(0,10)
          } else if (this.idPeriodo == 5) {
            if (element.recupP5_Ini != null) fechitaI = element.recupP5_Ini.substr(0,10)
            if (element.recupP5_Fin != null) fechitaF = element.recupP5_Fin.substr(0,10)
          }
