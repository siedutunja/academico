<template>
  <div>
    <b-row v-if="!validaVigencia">
      <b-col>
        <b-card>
          <b-alert variant="danger" show>
            <h5 class="mt-3">Vigencia no válida.</h5>
            <h6 class="mt-3">Lo sentimos, no se puede renovar matriculas en la vigencia {{ $store.state.aLectivo}}.</h6>
            <h6 class="mt-3">Cambie a la vigencia {{ $store.state.aMatriculas}} para renovar matriculas.</h6>
          </b-alert>
        </b-card>
      </b-col>
    </b-row>
    <b-row v-else>
      <b-col lg="12">
        <b-card>
          <template #header>
            <h5 class="mb-0"><b-icon icon="search" aria-hidden="true"></b-icon> RENOVACIÓN DE MATRICULAS PARA LA VIGENCIA {{ $store.state.aLectivo}}</h5>
          </template>
          <b-card-text>
            <b-row>
              <b-col>
                <p>Para realizar la revación de la matricula de un estudiante, este debe estar matriculado en la vigencia {{ vigenciaBusqueda }}.</p>
              </b-col>
            </b-row>
            <b-row class="mt-3">
              <b-col lg="6">
                ...
                <vue-good-table :columns="encabColumnas" :rows="listaEstudiantes" styleClass="vgt-table condensed bordered striped" :line-numbers="true"
                  ...
                  <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field == 'estudiante'">
                      <span style="font-weight: bold; color: blue; cursor: pointer" @click="renovarMatricula(props.row)">{{props.row.estudiante}}</span> 
                    </span>
                  </template>
                  <div slot="emptystate">
  ...
  export default {
    ...
    data () {
      return {
        ...
        encabColumnas : [
          ...
          { label: 'Observaciones/Pendientes', field: 'obs_matricula', sortable: true },
    ...
    methods: {
      ...
      async buscarEstudiante() {
        ...
        if (this.$v.buscarTexto.$anyError) {
          ...
          await axios
          .get(CONFIG.ROOT_PATH + 'academico/buscarestudiante/renovacion', { params: { texto: this.buscarTexto.textoBusqueda, idInstitucion: this.$store.state.idInstitucion, vigencia: this.vigenciaBusqueda }})
          .then(response => {
    ...
    beforeMount() {
      if(this.$store.state.idRol == 1 || this.$store.state.idRol == 12 || this.$store.state.perMatricular == 1) {
        this.validaVigencia = this.$store.state.aMatriculas == this.$store.state.aLectivo ? true : false
        this.vigenciaBusqueda = this.$store.state.aMatriculas - 1
      } else {
