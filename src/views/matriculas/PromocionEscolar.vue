                ...
                <vue-good-table ref="table" :columns="encabColumnas" :rows="estadoFinalPorEstudiante" styleClass="vgt-table condensed bordered striped" :line-numbers="true">
                  <template slot="table-row" slot-scope="props">
                    ...
                    <span v-if="props.column.field == 'idEstadoFinal'">
                      <b-form-select v-model="props.row.idEstadoFinal" @change="actualizarItem(props.row)" :options="comboEstadosFinales" disabled></b-form-select>
                    </span>
    ...
    data () {
      return {
        ...
        encabColumnas: [
          ...
          { label: 'Estado Promoción', field: 'idEstadoFinal', sortable: false },
          { label: 'Areas Perd.', field: 'perdidas', sortable: false },
          { label: '', field: 'estadoPromocion', sortable: false },
        ],
        idEstadoFinal: null,
        comboEstadosFinales: [],
      }
        ...
        this.listaEstudiantes.forEach(est => {
          ...
          this.listaAreasAsignaturas.forEach(asig => {
              ...
              if (asig.orden === 99) {
                ...
                if (this.$store.state.idInstitucion == 'eb58bf60-fc83-11ec-a1d1-1dc2835404e5') { //INEM
                  if (asig.orden == 55) {
                    ...
                    if (nota.periodo == 1) sumaNotas += parcial * 30 / 100
                    if (nota.periodo == 2) sumaNotas += parcial * 35 / 100
                    if (nota.periodo == 3) sumaNotas += parcial * 35 / 100
                  }
                } else if (this.$store.state.idInstitucion == 'acaa36d0-fcb1-11ec-8267-536b07c743c4') { //EMILIANI
                  if (asig.orden == 12) {
                      sumaNotas += parcial
                  } else {
                    if (nota.periodo == 1) sumaNotas += parcial * 20 / 100
                    if (nota.periodo == 2) sumaNotas += parcial * 20 / 100
            ...
            if (asig.orden === 99) {
              ...
              notaFinal = (!isNaN(habilitacion) && habilitacion > promedioAsignatura)
                ? habilitacion
                : promedioAsignatura
            ...
            asignaturas.forEach(asig => {
              ...
              if (this.$store.state.idInstitucion == 'eb58bf60-fc83-11ec-a1d1-1dc2835404e5') { //INEM
                ...
                if (asig.orden == 55) {
                  ...
                  promedioAsignatura = sumaNotas > 0 ? this.redondear(sumaNotas / 4).toFixed(1) : 0
                } else {
                  notasDeAsig.forEach(nota => {
                    let base = parseFloat(nota.definitiva) || 0
                    const recu = parseFloat(nota.recuperacion)
